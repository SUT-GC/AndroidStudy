...
public class ItemUtil {
	...
	public static List<Map<String,Object>> getAllItems(){
		...
		while(cursor.moveToNext()){
			...
			item.put("title",Base64Util.base64DecodeByString(cursor.getString(1)));
			item.put("content",Base64Util.base64DecodeByString(cursor.getString(2)));
	...
	public static void insertItem(Item item){
		String sql = "insert into node (" +
					...
					")values('"
					+Base64Util.base64EncodeByString(item.getTitle())+"','"
					+Base64Util.base64EncodeByString(item.getContent())+"','"
					+item.getDatetime()+"','"
					+item.getLabel()+"','"
					+item.getPower()+
					"')";
	...
	public static Item selectItemById(int id){
		...
		while (cursor.moveToNext()){
			...
			item.setTitle(Base64Util.base64DecodeByString(cursor.getString(1)));
			item.setContent(Base64Util.base64DecodeByString(cursor.getString(2)));
	...
	public static void updateItemById(int id, Item item){
		String sql = "update node set " +
						" title = '"+Base64Util.base64EncodeByString(item.getTitle())+"'," +
						" content = '"+Base64Util.base64EncodeByString(item.getContent())+"'," +
						" label = '"+item.getLabel()+"'" +
					 " where _id = '"+id+"'";
		sqLiteDatabase.execSQL(sql);
	}
